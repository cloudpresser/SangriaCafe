...
const Cart = (props) => {
  ...
  /**
  * This function sets the timer on screen
  */
  const showTimer = async () =>{
    // gets lastorder from local storage
    let order = await AsyncStorage.getItem('lastorder')
    order = JSON.parse(order)
    // if lastorder.time + 55 > now 
    let futureTime = new Date(new Date(order.time).getTime() + 55*60000)
    if(futureTime>new Date()){
      ...
      // sets order timer, address and price
      setInterval(()=>{
        // timer calculations
        let timeDiffMilli = futureTime - new Date()
        let diff = new Date(timeDiffMilli)
        const timeDiffMin = diff.getMinutes()
        const timeDiffSec = diff.getSeconds()
        const remainingTime = `${timeDiffMin}:${addZero(timeDiffSec)}`
  ...
  /**
  * This function stores the timer, and shows it on screen
  */
  const storeOrder = async (time, address, price, cart) => {
    // create order object
    const order = {
      time, 
      address, 
    ...
    result = await AsyncStorage.setItem('lastorder', JSON.stringify(order) )
    .then( ()=>{
      return order
    } )
    .catch( ()=>{
      throw 'There was an error saving the product'
    } )
    // show timer
  ...
  return props.foodCart.length > 0 ? (
      ...
      <SafeAreaView>
        ...
        <ScrollView
          ...
          {props.foodCart.map((food) => {
            return (
              <View
                style={styles.recieptTheme}
                key={food.item[0] + food.instruction}>
                <View>
                  ...
                  <Text style={styles.subText}>
                    {food.instruction ? food.instruction : null}
                  </Text>
                </View>
                <View>
    ...
    <>
      <SafeAreaView>
        ...
        {lastOrder&&(
          <>
            <Text>Thank you, your order is on the way. It should arrive in {lastOrder.timeLeft}</Text>
            <Text>{lastOrder.address.street + lastOrder.address.postalCode}</Text>
            <Text>{lastOrder.cart.map(item=>`${item.item[0]} ${item.quantity}`)}</Text>
            <Text>${lastOrder.price}</Text>
          </>
        )}
          <Image
            source={{
